...
(function(){
    ...
    p.init = function(){
        this.type1 = $('<option>请选择经销商</option>').val("-1").data({"callback":true});
        this.type2 = $('<option>请选择服务商</option>').val("-1").data({"callback":true});
    ...
    p.initEvent = function(){
        ...
        $("select").change(function(){
            ...
            if($optSelected=="省"){
                $(".city select").empty();
                $(".city select").append($('<option>城市</option>').val("-1").data({"callback":true}));
        ...
        geolocation.getCurrentPosition(function (r) {
            if (this.getStatus() == BMAP_STATUS_SUCCESS) {
                ...
                $.ajax({
                    ...
                    data:{"lon":lngVal,"lat":latVal,"type":$("input.business_type").val()},
                    type:"get",
    ...
    p.initMap = function(obj) {
        ...
        if (obj.length == "search") {
            ...
            var opts1 = {width: 260, title: '<span style="font-size:14px;color:#0A8021">' + obj.countryName + '</span>'};
            ...
            var infoWindow1 =new BMap.InfoWindow("<div style='line-height:1.5;font-size:12px;'><b>地址: </b>"+obj.adressName+"</br><b>电话: </b><a style='color:#00c9ff;' href='tel:"+obj.phoneNum+"'>"+obj.phoneNum+"</a></br><a href='"+useHref+"' style='color:#0066b3; font-size: 0.16rem;' >路线查询</a>"+testDriver_a+"</div>", opts1);
        ...
        if (obj.id) {
            ...
            var $comp = $('<p class="company_name">'+val.dealerName+'</p>'
                + ' <p class="company_address">地址：'+val.dealerAddress+'</p>'
                + '<p>电话：<a tel="'+val.dealerTel+'" class="tel_num">'+val.dealerTel+'</a></p>');
    ...
    p.initmapDetail = function(val,map){
        // 用经纬度设置地图中心点
        var latVal = val.longitudeLatitude.split("_")[0];
        var lngVal = val.longitudeLatitude.split("_")[1];
        var myIcon = new BMap.Icon("../../static/mStatic/images/support/foton_logo.jpg", new BMap.Size(30,30));
        var new_point = new BMap.Point(latVal,lngVal);
        var marker = new BMap.Marker(new_point,{icon:myIcon});  // 创建标注
        map.addOverlay(marker);              // 将标注添加到地图中
        ...
        var opts1 = {width : 260, title : '<span style="font-size:14px;color:#0A8021">'+val.dealerName+'</span>'};
        //调用地址
    ...
    p.initDealer = function(data){
        ...
        if(data.length=="0"){
            ...
            $(".distributor select").append($('<option>'+name_type+'</option>').val("-1").data({"callback":true}));
            //return;
        }
        $.each(data,function(i,val){
            var $option = $('<option title='+val.dealerName+'>'+val.dealerName+'</option>').val(val.longitudeLatitude).data({"callback": true,"provinceId":val.provinceId,"cityId":val.cityId,"areaId":val.areaId,"address":val.dealerAddress,"name":val.dealerName,"phone":val.dealerTel,"id":val.id});
            $option.bind("callback",function(){
